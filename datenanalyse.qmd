...
```{r}
# Zeilen mit fehlenden Werten ermitteln
...
```{r}
penguins_0 <- penguins |> 
  mutate(body_mass_g = replace_na(data = body_mass_g, replace = 0))
...
# NA in einer Spalte ersetzen
penguins_val <- penguins |> 
  mutate(body_mass_g = replace_na(data = body_mass_g, replace = mean_weight))

# NA in mehreren Spalten ersetzen
penguins_val <- penguins_val |> 
  replace_na(list(bill_length_mm = 0, bill_depth_mm = 0))
...
```{r}
# Spalte hinzufügen und Reihenfolge ändern
penguins |> 
...
# Theme Datenpfad für ggplot2 erstellen

# Konstante
schrift_farbe <- "#262B32"
...
theme_dv <- function(...) {
  theme(
    ...
    # Kleine Mehrfachdarstellungen (Facetten)
...
In R können mit den Funktionen `group_by()` und `summarise()` gruppierte Zusammenfassungen erstellt werden. Dabei ändert `group_by()` die Analyseeinheit vom gesamten Datensatz zu einzelnen Gruppen, während `summarise()` einen neuen Datenrahmen für die zusammenfassende Statistik erstellt. Alternativ kann die Gruppierungsvariable über das Argument «.by» bestimmt werden.
...
```{r}
penguins_def |> 
  summarise(Mittelwert_Gewicht = mean(body_mass_g), .by = species) |> 
  arrange(desc(Mittelwert_Gewicht))
```
...
```{r}
# Mithilfe von group_split() einen Datenrahmen gruppieren und aufteilen
penguins_def |> 
  group_split(species) |> 
  map(
    \(df_group) 
    lm(formula = flipper_length_mm ~ bill_length_mm, data = df_group))
```

```{r}
# Alternative Variante mit der reframe-Funktion von dplyr
penguins_def |> 
  mutate(species = paste0("species_", species)) |> 
  reframe(lm(formula = flipper_length_mm ~ bill_length_mm) |> 
  broom::tidy(), 
  .by = species)
```
...
penguins_long |> 
  ...
  scale_fill_paletteer_d(palette = "nbapalettes::pacers_classic", 
                         labels = c("Gewicht", "Flossenlänge")) +
...
```{r}
# Ausgabe der Ergebnisse in wissenschaftlicher Notation deaktivieren
options(scipen = 999)
```
...
#### Kombiniertes Histogramm und Dichtediagramm
...
penguins_def |> 
  ...
  # Spalte "species" entfernen, um Teilmenge zu vermeiden
  geom_point(data = select(.data = penguins_def, -species), 
             mapping = aes(size = body_mass_g), 
             colour = "grey85", 
             alpha = 0.7) +
  ...
  labs(title = "Streudiagramm mit Facetten", 
       x = "Schnabellänge",
       y = "Schnabeltiefe") +
  theme_minimal(base_size = 10) +
  theme_dv(legend.position = "none") +
  theme(
    # Abstände zwischen Panels vergrössern
    panel.spacing.x = unit(x = 30, units = "pt")
  )
```
...
```{r}
# Hilfsfunktion zur Erstellung der Streudiagramme
scatter_fn <- function(df, col1, col2, title) {
  df |> 
    ggplot(mapping = aes(x = {{ col1 }}, y = {{ col2 }})) +
    geom_point() +
...
```{r}
# Pinguine nach Arten gruppieren und eine Listenspalte mit Plot-Objekten hinzufügen
penguin_scatter <- penguins_def |> 
  nest_by(species) |> 
...
```{r}
# Verschachtelten Datenrahmen ausgeben
glimpse(penguin_scatter)
```

```{r}
for (i in 1:3) {
  assign( # Weist einem Namen in der Umgebung einen Wert zu
    x = paste("plot", i, sep = "_"),
    value = penguin_scatter$plot[i][[1]]
  )
}
```
...
(plot_1 + plot_2 + plot_3) +
  plot_annotation(title = "Streudiagramm mit Teilmengen", 
                  tag_levels = "1", 
                  tag_prefix = "1.", 
                  tag_suffix = ".")
```
...
p1 <- penguins_def |> 
  ...
  labs(x = "Schnabellänge",
       y = "Schnabeltiefe", 
       color = "") +
  theme_minimal(base_size = 10) +
  theme_dv(legend.position = "inside", 
           legend.position.inside = c(0.975, 1))
```
...
# Histogramm 2
p3 <- penguins_def |> 
  ggplot(mapping = aes(y = bill_depth_mm)) +
  geom_histogram(mapping = aes(fill = species), 
                 binwidth = 0.5) +
  scale_fill_manual(values = palette, 
                    guide = "none") +
...
```{r}
# Streudiagramm und beide Histogramme mit patchwork kombinieren
...
<p>&nbsp;</p>
![](www/header-logo.png){height="1.05cm"}

<https://www.datenpfad.ch>
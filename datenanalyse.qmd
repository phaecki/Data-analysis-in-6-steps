...
Die explorative Datenanalyse (Exploratory Data Analysis, abgekürzt EDA) ist ein wesentlicher Schritt in jedem Datenanalyseprojekt. Sie dient der Analyse und Untersuchung von Datensätzen und der Zusammenfassung ihrer wichtigsten Merkmale, wobei häufig grafische Darstellungsmethoden verwendet werden. Mit Hilfe von Tabellen, Grafiken und der Ermittlung relevanter Kennzahlen wird versucht, einen Überblick über das gesamte Datenmaterial zu gewinnen, es zu ordnen und zusammenzufassen. Die EDA bildet damit die Grundlage für die weitere Analyse.
...
```{r}
# Nominalskalierte (kategoriale) und metrische Variable
plot(penguins$species, ylim = c(0, 200), col = "grey", border = NA)
```

```{r}
# Gruppiertes Säulendiagramm
barplot(prop.table(table(penguins$species, penguins$sex), margin = 2), legend.text = TRUE, beside = TRUE, xlim = c(0, 11), ylim = c(0, 0.5))
```

```{r}
# Säulen mit unterschiedlichen Farben
barplot(tapply(X = penguins$body_mass_g, INDEX = penguins$species, FUN = mean, na.rm = TRUE), col = c("#9FC131", "#93257B", "#57AF2C"), ylim = c(0, 6000))
```
...
```{r}
pairs.panels(x = penguins[, 3:6], hist.col = "#9FC131")
```
...
# Konstante
default_font_color <- "#2d2926"

theme_dv <- function(...) {
  theme_minimal(base_size = 16) +
  theme(text = element_text(family = "Helvetica", 
                            colour = default_font_color), 
        ...
        # Titel
        legend.text = element_text(colour = default_font_color, 
                                   size = rel(0.7), 
                                   hjust = 0), 
        ...
        # Achsenbeschriftung
        axis.title = element_text(colour = default_font_color, 
                                  size = rel(0.8), 
                                  hjust = 0.5), 
        axis.text = element_text(colour = default_font_color, 
                                 size = rel(0.7)), 
...
lapply(X = zahlen, FUN = split_number)
```
...
round(x = prop.table(table(penguins_def$island)) * 100, digits = 2)
```
...
round(x = prop.table(table(penguins_def$species, penguins_def$island), 
                     margin = 1) * 100, 
      digits = 2)
```
...
penguins_def %>% 
  ...
  labs(title = "Datenverteilung", 
       x = "Wert", 
       y = element_blank()) +
  theme_dv(panel.grid.major.y = element_blank(), legend.position = "none") +
